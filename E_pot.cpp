...
double E_pot::oscillator()
{
    ...
    for (int i = 0; i < m_number_particles; i++) {
      ...
      for (int j = 0; j < m_dimension; j++) {
        m_r_single_particle += m_r(i,j)*m_r(i,j);
      }
      m_e_potential_harm += m_r_single_particle*m_omega*m_omega/2.0;
    }
    return m_e_potential_harm;
}
...
//the classe of the potential energy should work for any number of particles
